...
public class ExerciseActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_test_questions_view);
	...
	private void initView() {
		...
		for (int i = 0; i < exercises.size(); i++) {
			...
			final int no = i + 1;
			LinearLayout questionContainer = (LinearLayout) view.findViewById(R.id.question);
			if (exercises.get(i).getTag()==1) {//此时的题目为图片
				TextView order = new TextView(this);
				order.setText((i + 1) + ". ");
				ImageView titleName = new ImageView(this);
				ImageLoaderUitil.display(exercises.get(i).getTitle(), titleName, this);
				questionContainer.addView(order);
				questionContainer.addView(titleName);
			}else {
